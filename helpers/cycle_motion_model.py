...
def find_optimal_motion(context: bpy.types.Context):
    """Zyklisches Durchtesten aller verfügbaren Motion-Models (für neue Marker)."""
    clip = getattr(context.space_data, "clip", None) or getattr(context.scene, "active_clip", None)
    if not clip:
        print("[Fehler] Kein aktiver Clip gefunden.")
        return None
    ...
    # Alle verfügbaren Motion-Modelle abrufen
    ...
    for modell in models:
        settings.default_motion_model = modell
        _, _, score = evaluate_tracking(context)
        if score > bester_score:
            bester_score = score
    ...
    if bestes_modell:
        settings.default_motion_model = bestes_modell
        print(f"[Info] Bestes Motion Model gesetzt: {bestes_modell}")
