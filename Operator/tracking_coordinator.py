# Helper/jump_to_frame.py — Original + minimale Signalerweiterung
import bpy
...
def run_jump_to_frame(
    ...
    *,
    ...
    # --- NEU (minimal-invasiv, NUR nach sicherer Playhead-Setzung): Optimizer-Signal ---
    #    Wenn derselbe Frame > 5× angesprungen wurde und scene["marker_adapt"] > 200,
    #    wird ein einmaliges Signal an den Koordinator gelegt. Sonst bleibt alles unverändert.
    if repeat_count > 5:
        try:
    ...
    # NEU: Sättigungs-Schwelle (10) – unverändert beibehalten
    repeat_saturated = repeat_count >= 4
    
    # REPEAT-HOOK: Bei Wiederholung (Frame wurde schon einmal per Jump angefahren)
    # → Nur noch run_marker_adapt_boost() ausführen. (unverändert beibehalten)
    if repeat_count >= 2:
