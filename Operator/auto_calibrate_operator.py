...
class KAISERLICHTRACKER_OT_auto_calibrate(bpy.types.Operator):
            ...
            for idx, s in enumerate(stages, start=1):
                ...
                if th_new <= self.MIN_THRESHOLD:
                    self._log(
                        f"{prop_name}: Stufe {idx} ({s:+.0%}) → Untergrenze erreicht ({th_new:.6f}), Abbruch."
                    )
                    break
                ...
                if sgn > sg:
                    self._log(
                        f"{prop_name}: Stage {idx} ({s:+.0%}) → Verbesserung ({sgn:.2f} > {sg:.2f}) → neuer Wert {th_new:.6f}"
                    )
                    sg = sgn
                    ...
                    self._log(
                        f"{prop_name}: Stage {idx} ({s:+.0%}) → keine Veränderung ({sgn:.2f} == {sg:.2f})"
                    )
                else:
                    self._log(
                        f"{prop_name}: Stage {idx} ({s:+.0%}) → Verschlechterung ({sgn:.2f} < {sg:.2f}) → weiter zur nächsten Stufe"
                    )
                    continue
            ...
            self._log(
                f"{prop_name}: Fertig. Bester Wert {best_value:.6f}, neue Baseline {baseline_length:.2f}"
            )
