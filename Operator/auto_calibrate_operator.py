...
from ..Helper.ui_refresh import refresh_clip_editor   # 🔄 Nur Movie-Clip-Editor (Viewer/Graph/Dopesheet/Timeline) refreshen
...
def short_test_track(
    ...
    refresh_clip_editor(context)  # 🔄 Startzustand sichtbar machen
    ...
    try:
        ...
        if tracks_to_delete:
            ...
            if names:
                ...
                refresh_clip_editor(context)  # 🔄 nach explizitem Delete
        ...
        if start_frame is not None:
            try:
                ...
                pass
            refresh_clip_editor(context)  # 🔄 nach Reset

        # 6) FINAL: Gesamtlänge (VOR Delta-Cleanup)
        try:
        ...
        refresh_clip_editor(context)  # 🔄 nach Längenberechnung
        ...
        try:
            ...
            if new_names:
                ...
                refresh_clip_editor(context)  # 🔄 nach Delta-Delete
        except Exception:
            pass
...
def short_test_pipeline(context=None, tracks_to_delete=None, report_fn: Optional[Any] = None):
    ...
    try:
        ...
        refresh_clip_editor(context)  # 🔄 Props gesetzt
        rb = short_test_track(
            context=context,
        ...
        try:
            ...
            pass
        refresh_clip_editor(context)  # 🔄 nach Baseline-Result
    except Exception:
        pass
...
def _restore_thresholds(scene: bpy.types.Scene, snap: Dict[str, float]) -> None:
    for k, v in snap.items():
        try:
            ...
            pass
    refresh_clip_editor(bpy.context)  # 🔄 nach kompletter Wiederherstellung
    ...
    try:
        for i, cfg in enumerate(grid):
            ...
            try:
                ...
                refresh_clip_editor(context)  # 🔄 nach Setzen der Kandidaten
            ...
            try:
                ...
                refresh_clip_editor(context)  # 🔄 nach jedem Messlauf
...
def deep_test_rot_xy(context: Optional[bpy.types.Context] = None,
                     grid: Optional[List[Tuple[float, float]]] = None,
                     tracks_to_delete: Optional[List[str]] = None) -> Dict[str, Any]:
    if grid is None:
...
def deep_test_scale_min_max(context: Optional[bpy.types.Context] = None,
                            grid: Optional[List[Tuple[float, float]]] = None,
                            tracks_to_delete: Optional[List[str]] = None) -> Dict[str, Any]:
    if grid is None:
...
def deep_test_rot_scale_pair(context: Optional[bpy.types.Context] = None,
                             grid: Optional[List[Tuple[float, float]]] = None,
                             tracks_to_delete: Optional[List[str]] = None) -> Dict[str, Any]:
    if grid is None:
...
def deep_test_perspective(context: Optional[bpy.types.Context] = None,
...
def reduce_threshold_single(context: Optional[bpy.types.Context],
                            prop_name: str,
                            cfg: ReduceConfig,
                            tracks_to_delete: Optional[List[str]] = None,
                            report_fn: Optional[Any] = None) -> Dict[str, Any]:
    scene = (context.scene if context is not None else bpy.context.scene)
    snap = _snapshot_thresholds(scene)
    ...
    try:
        ...
        while sf >= 1.0 and outer < cfg.max_outer_iters:
            ...
            while inner < cfg.max_inner_iters:
                ...
                refresh_clip_editor(context)  # 🔄 Kandidat im UI
                res = short_test_track(
                    context=context, tracks_to_delete=tracks_to_delete,
                    run_meta={"sf": sf, "fields": [prop_name], "tag": f"Reduce {prop_name}"},
                    report_fn=report_fn
                )
                refresh_clip_editor(context)  # 🔄 nach Messung
                ttl = int(float(res.get("total_track_length", 0.0)))
                logs.append({"sf": sf, "threshold": candidate})
                if _is_success(ttl, cfg.target_len):
                    if ttl > best_len:
                        best_len = ttl; best_val = candidate; best_sf = sf
...
def reduce_threshold_single_with_extras(context: Optional[bpy.types.Context],
                                        prop_name: str,
                                        cfg: ReduceConfig,
...
def reduce_threshold_pair(context: Optional[bpy.types.Context],
    ...
    try:
        ...
        while sf >= 1.0 and outer < cfg.max_outer_iters:
            outer += 1
            prev_a = current_start_a; prev_b = current_start_b
            had_success = False
            next_start_after_stage: Optional[Tuple[float, float]] = None
            inner = 0
            while inner < cfg.max_inner_iters:
                inner += 1
                cand_a = prev_a / sf; cand_b = prev_b / sf
                ...
                if _is_success(ttl, cfg.target_len):
                    if ttl > best_len:
                        best_len = ttl; best_pair = (cand_a, cand_b); best_sf = sf
                    last_success_prev_global = (prev_a, prev_b)
                    next_start_after_stage = (prev_a, prev_b)
                    had_success = True
                    break
                else:
                    prev_a = cand_a; prev_b = cand_b
            if had_success and next_start_after_stage is not None:
...
def reduce_threshold_rot_xy_coupled(context: Optional[bpy.types.Context],
                                    prop_x: str, prop_y: str,
                                    hw_ratio: float,
                                    cfg: ReduceConfig,
                                    tracks_to_delete: Optional[List[str]] = None,
                                    report_fn: Optional[Any] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        while sf >= 1.0 and outer < cfg.max_outer_iters:
            ...
            while inner < cfg.max_inner_iters:
                ...
                if _is_success(ttl, cfg.target_len):
                    if ttl > best_len:
                        best_len = ttl; best_x = cand_x; best_y = cand_y; best_sf = sf
...
def reduce_threshold_scale_coupled(context: Optional[bpy.types.Context],
                                   prop_min: str, prop_max: str,
                                   factor: float,
                                   cfg: ReduceConfig,
                                   tracks_to_delete: Optional[List[str]] = None,
                                   report_fn: Optional[Any] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        while sf >= 1.0 and outer < cfg.max_outer_iters:
            ...
            while inner < cfg.max_inner_iters:
                ...
                if _is_success(ttl, cfg.target_len):
                    if ttl > best_len:
                        best_len = ttl; best_min = cand_min; best_max = cand_max; best_sf = sf
...
def reduce_rot_scale_pair(context, target_len: int, start: Tuple[float, float] = (1.0, 1.0), report_fn=None, **kw):
    ...
    refresh_clip_editor(context)  # 🔄 nach Stage 1

    best_rot = float(res_rot.get("best", {}).get("value", start[0]))
    # Stage 2: scale-only (rot=0), Start = start[1]
    ...
    refresh_clip_editor(context)  # 🔄 nach Stage 2
...
class KAISERLICHTRACKER_OT_auto_calibrate(bpy.types.Operator):
    ...
    def execute(self, context):
        try:
            ...
            refresh_clip_editor(context)  # 🔄 nach Setzen aller Thresholds
            ...
            try:
                ...
                refresh_clip_editor(context)  # 🔄 nach Pipeline
            ...
            try:
                ...
                # STEP1 → Rot/XY (gekoppelt; nur X-Reduktion)
                if "STEP1" in ge_list:
