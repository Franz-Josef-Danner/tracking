...
class CLIP_OT_detect_cycle(Operator):
    ...
    def execute(self, context):
        ...
        # Operative Parameter
        # Margin aus marker_helper_main (Scene-Keys); Fallback aus Clipbreite
        ...
        # min_distance: bevorzugt zuletzt gestufter Wert, sonst Basiswert
        ...
        # Für dieses Cleanup den aktuellen md px sichtbar machen
        try:
        ...
        # 2) Distance-Cleanup mit Baseline
        dist_res = run_distance_cleanup(context, baseline_ptrs=pre_ptrs, frame=frame)
        ...
        if isinstance(raw_after, (list, tuple, set)):
            ...
            # Fallback: alles was neu ist und am Frame einen Marker besitzt
            post_ptrs = self._tracks_with_marker_at_frame(context, frame)
            new_after = {p for p in post_ptrs if p not in pre_ptrs}

        # 4) Anzahl evaluieren und ggf. bei TOO_MANY reduzieren
        ...
        if eval_res.get("status") == "TOO_MANY":
            ...
            if to_delete > 0:
                ...
                # nach Fehlerwert sortieren (schlechteste zuerst löschen)
                try:
                ...
                for t in cand_tracks[:to_delete]:
                    try:
                        ...
                        pass
                # Nach dem Löschen Status als ENOUGH markieren mit gekappter Anzahl
                eval_res = {
                    "status": "ENOUGH",
