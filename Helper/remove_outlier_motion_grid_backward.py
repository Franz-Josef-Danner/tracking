# Helper/remove_outlier_motion_grid_backward.py
# -------------------------------------------------------------------------
import bpy
import math
...
# ------------------------------------------------------------
# INTERNAL: collect tracks with continuous history at f+k → f
# ------------------------------------------------------------
def _collect_tracks_backward(tracks, f, k, endf):
    valid = []
    fk = f + k
    if fk > endf:
        return valid
...
def _evaluate_tracks(track_data, max_error):
    """
    track_data: list[(track, p_now, p_future)]
    Bewegung: f+k -> f
    """
    flagged = set()
    ln = len(track_data)
    ...
    for i in range(ln):
        ...
        # Vektor: von Zukunft in Gegenwart
...
# ------------------------------------------------------------
def remove_outlier_motion_grid_backward(context: bpy.types.Context):
    """
    Löscht Tracks mit unrealistischen Bewegungen (Rückwärts-Auswertung)
    1) global
    2) 4er Raster
    3) 16er Raster

    Frames vorwärts: scene["kaiserlich_frames_per_track"] / 4
    Threshold in Pixel: scene["max_error_value"]
    Bewegung geprüft: f+k -> f
    """
    scene = context.scene
    ...
    endf = clip.tracking.frames_end if hasattr(clip.tracking, "frames_end") else scene.frame_end

    # ----------------------------------------
    # COLLECT VALID TRACKS
    # ----------------------------------------
    td = _collect_tracks_backward(tracks, f, k_frames, endf)
    if len(td) < 2:
        return {"CANCELLED"}
