...
def run_detect_adapt(context: bpy.types.Context) -> None:
    ...
    while loop < max_loops:
        ...
        # Bewertung
        if remaining == 0:
            print("[DetectAdapt] ⚠️ Keine gültigen Marker – nächster Versuch.")
        elif abs(diff) <= tolerance and remaining > 0:
            break
        else:
            print(f"[DetectAdapt] Δ={diff:+.0f}, Ziel={ef_target}, Toleranz={tolerance:.1f}")

        # Dynamische Anpassung min_distance
        if remaining == 0:
