...
def apply_formula_on_selected_tracks_backwards(context: bpy.types.Context, max_frames: int = 10) -> None:
    ...
    try:
        ...
        global_model = _evaluate_motion_model_pairwise_backwards(
            all_positions,
            getattr(scene, "kaiserlich_rot_thresh_x", 0.002)  / 125,
            getattr(scene, "kaiserlich_scale_thresh_max", 0.005) / 5000,
            getattr(scene, "kaiserlich_rot_scale_thresh_rot", 0.002) / 100,
            getattr(scene, "kaiserlich_rot_scale_thresh_scale", 0.005) / 2500
        )

        # --- 2) Perspective global & per Marker einmalig berechnen ---
        _, global_p_dev, per_marker_dev = _detect_perspective_motion(
            marker_positions,
            getattr(scene, "kaiserlich_perspective_thresh", 0.002) / 5
        )
        perspective_thresh = getattr(scene, "kaiserlich_perspective_thresh", 0.002) / 5
