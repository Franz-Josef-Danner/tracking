...
def apply_formula_on_selected_tracks(context: bpy.types.Context, max_frames: int = 10) -> None:
    ...
    try:
        ...
        global_model = _evaluate_motion_model_pairwise(
            all_positions,
            getattr(scene, "kaiserlich_rot_thresh_x", 0.002) / 1000,
            getattr(scene, "kaiserlich_scale_thresh_max", 0.005) / 1000000,
            getattr(scene, "kaiserlich_rot_scale_thresh_rot", 0.002) / 1000,
        ...
        _, global_p_dev, per_marker_dev = _detect_perspective_motion(
            marker_positions,
            getattr(scene, "kaiserlich_perspective_thresh", 0.002) / 1000
        )
        perspective_thresh = getattr(scene, "kaiserlich_perspective_thresh", 0.002) / 1000
