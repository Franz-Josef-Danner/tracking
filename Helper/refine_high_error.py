...
class CLIP_OT_refine_high_error_modal(Operator):
    ...
    # Hilfsfunktion: Ziele bestimmen (Solve-Error als Untergrenze)
    def _prepare_targets_after_scan(self) -> None:
        ...
        print(f"[RefineModal] Solve-Error (threshold) = {thr:.6f}; selected {len(self._targets)}/{len(pairs)} frames")
...
class CLIP_OT_refine_high_error_modal(Operator):
    ...
    def _prepare_targets_after_scan(self) -> None:
        # Sortiere Szene-Frames nach Fehler-Summe absteigend
        pairs = sorted(self._scan_errors.items(), key=lambda kv: kv[1], reverse=True)
        # nimm nur Frames mit > 0 Fehler und begrenze auf top_n_frames
        filtered = [f for (f, s) in pairs if s > 0.0]
        if not filtered:
            # falls alle 0.0 sind, nimm trotzdem die ersten N
