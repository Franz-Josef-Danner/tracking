...
def _get_clip_and_objects(context: bpy.types.Context):
    clip = (
    ...
    if obj is None:
        raise RuntimeError("No MovieTrackingObject found in clip.tracking.objects.")
    return clip, tr, obj
...
def _clip_frame_range(clip):
    ...
    frames = []
    mt = clip.tracking
    tracks = mt.objects.active.tracks if getattr(mt.objects, "active", None) else mt.tracks
    for t in tracks:
        for mk in t.markers:
            frames.append(mk.frame)
    if frames:
...
def compute_parallax_scores(obj, clip, delta: int = 5):
    tracks = [t for t in obj.tracks if len(t.markers) > 2 * delta]
...
def collect_metrics(clip, obj, holdouts: Iterable[bpy.types.MovieTrackingTrack], *, center_box: float = 0.6):
    ...
    fs, fe = _clip_frame_range(clip)
    total = fe - fs + 1
    lens = [(t.markers[-1].frame - t.markers[0].frame + 1) for t in obj.tracks if len(t.markers) >= 2]
...
def run_solve_eval(context, config: SolveConfig) -> tuple[str, SolveMetrics, list[SolveMetrics]]:
    ...
    tracks = [t for t in obj.tracks if len(t.markers) >= 2]
    if len(tracks) < 40:
    ...
    fs, fe = _clip_frame_range(clip)
    scores = compute_parallax_scores(obj, clip, delta=config.parallax_delta)
    if scores:
        f = scores[0][0]
        a = max(int(f - config.parallax_delta), int(fs))
        b = min(int(f + config.parallax_delta), int(fe))
        obj.keyframe_a = a
        obj.keyframe_b = b
    ...
    keyframe_a = int(getattr(obj, "keyframe_a", fs))
    ...
    for model in ("POLYNOMIAL", "DIVISION", "BROWN"):
        ...
        for stage in range(1, config.refine_rounds + 1):
            ...
            hold_med, hold_p95, edge_gap, persist, lr_diff, tb_diff = collect_metrics(clip, obj, holdouts, center_box=config.center_box)
            try:
