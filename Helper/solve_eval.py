...
def run_solve_eval(context, config: SolveConfig) -> tuple[str, SolveMetrics, list[SolveMetrics]]:
    ...
    if scores:
        f = scores[0][0]
        tr_settings.keyframe_a = max(int(f - config.parallax_delta), int(fs))
        tr_settings.keyframe_b = min(int(f + config.parallax_delta), int(fe))
        tr_settings.use_keyframe_selection = False
    else:
        tr_settings.use_keyframe_selection = True

    keyframe_a = int(getattr(tr_settings, "keyframe_a", fs))
