...
def get_from_selected_tracks_backwards(
    ...
    try:
        ...
        scene["kaiserlich_rot_thresh_x"] = max(0, min(2, 5 - ((dx_var_mean * 0.0036) * 1100)))
        scene["kaiserlich_rot_thresh_y"] = max(0, min(2, 6 - ((dy_var_mean * 0.0067) * 1000)))
        scene["kaiserlich_scale_thresh_min"] = max(0, min(2, 3 - ((rel_var_mean * 0.00024) * 10000)))
        ...
        scene["kaiserlich_perspective_thresh"] = max(0, min(50, 50 - ((global_p_dev_accum_mean * 10) * 100)))
