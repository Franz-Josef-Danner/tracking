...
def detect_prepass_cleanup(context: bpy.types.Context, *, remove_names: Optional[Iterable[str]] = None) -> Dict[str, Any]:
    ...
    prev_raw = _safe_idprop_get_list(scn, DETECT_PREV_KEY)
    names_to_remove = set(prev_raw)
    if remove_names:
        names_to_remove.update(_coerce_utf8_str_list(remove_names))

    removed = 0
    if names_to_remove:
        removed = _remove_tracks_by_name(mc.tracking, names_to_remove)

    _try_set_scene_list(scn, DETECT_PREV_KEY, [])
    print(f"[DetectDebug] Pre-Cleanup: removed={removed} from prev_names={len(prev_raw)}")
