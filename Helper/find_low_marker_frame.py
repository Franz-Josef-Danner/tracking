...
def run_find_low_marker_frame(
    ...
    *,
    ...
    from .clean_error_tracks import CLIP_OT_clean_error_tracks  # ← Import direkt hier
    ...
    if low_frame is not None:
        ...
        print("[MarkerCheck] Keine Low-Marker-Frames gefunden. Starte Clean Error Tracks …")
        try:
            # Direkt den Operator ausführen
            CLIP_OT_clean_error_tracks().execute(context)
        except Exception as ex:
            print(f"Error: Clean Error Tracks fehlgeschlagen: {ex}")
            return None
        return None
