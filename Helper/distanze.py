...
- Im gleichen Frame alle weiteren *aktiven* (existieren) und *nicht gemuteten*
  Marker als "alt" erfassen.
...
Hinweise:
- Marker-Koordinaten (`MovieTrackingMarker.co`) sind *normalisiert* (0..1) relativ
  zur Clip-Breite/-Höhe. Pixel = (co.x * width, co.y * height).
- "Aktiv" bezieht sich hier auf: Für den aktuellen Frame existiert ein Marker
  (`markers.find_frame(frame, exact=True)` liefert ein Objekt) und er ist nicht
  gemutet (`marker.mute is False`).
- "Selektiert" bezieht sich auf die Marker-Selektion des Keyframes
  (`MovieTrackingMarker.select`).
- Es wird ein interner _solve_log()-Helper verwendet, der – falls vorhanden –
  die zentrale Log-Funktion des Add-ons aufruft; andernfalls wird in die Konsole
  gedruckt.

Rückgabe:
    Tuple[bool, dict]: (ok, info)
        ok   – True bei erfolgreicher Ausführung
        info – Strukturierte Daten zu Frame/Markerlisten (nützlich für Tests)
...
def _solve_log(context: bpy.types.Context, value: Any) -> None:
    """An zentrale Solve-Log-Funktion delegieren, falls vorhanden; sonst print()."""
    try:
...
def _marker_pixel_pos(marker: "bpy.types.MovieTrackingMarker", width: int, height: int) -> Tuple[float, float]:
    """Umrechnung von Normalized- zu Pixel-Koordinaten."""
    try:
        x = float(marker.co[0]) * float(width)
        y = float(marker.co[1]) * float(height)
        return x, y
    except Exception:
        return 0.0, 0.0
...
def run_distance_cleanup(context: bpy.types.Context) -> Tuple[bool, Dict[str, Any]]:
    """Erfasst selektierte (neu) und übrige aktive, ungemutete (alt) Marker im
    aktuellen Frame, berechnet Pixelpositionen und schreibt Log-Zeilen.
